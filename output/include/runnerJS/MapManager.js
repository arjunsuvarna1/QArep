...
Runner.controls.MapManager = Runner.extend( Runner.emptyFn, {
	...
	constructor: function( cfg ) {
		...
		if ( !Runner.isMobile ) {
			this.showViewInPopup = Runner.pages.PageSettings.getTableData(this.tName, "showViewInPopup");
			this.showEditInPopup = Runner.pages.PageSettings.getTableData(this.tName, "showEditInPopup");
		}
	...
	/**
	 *  Get an object providing map provider-specific operations
	 * @return {object}
	 */
	...
	initMap: function( mapId, onMapLoadHandler ) {
		...
		if ( (mapData.type == 'BIG_MAP' || mapData.type == 'DASH_MAP' ) && ( mapData.zoom === "auto" || typeof mapData.zoom === "undefined" ) ) {
	...
	getBoundMarkers: function( mapData ) {
		...
		for (i = 0; i < markers.length; i++) {
			if ( markers[i].detached || markers[i].lat == '' ||	markers[i].lng == '' || ( this.isHeatMap( mapData ) && mapData.weightField && markers[i].weight <= 0 ) ) {
				continue;
			}
			...
			if ( !markersExists ) {
				...
				minLatID = minLat > markers[ i ].lat ? i : minLatID; 
				minLngID = minLng > markers[ i ].lng ? i : minLngID;
				maxLatID = maxLat < markers[ i ].lat ? i : maxLatID;
				maxLngID = maxLng < markers[ i ].lng ? i : maxLngID;			
			
				minLat = minLat > markers[ i ].lat ? markers[ i ].lat : minLat; 
				minLng = minLng > markers[ i ].lng ? markers[ i ].lng : minLng;
				maxLat = maxLat < markers[ i ].lat ? markers[ i ].lat : maxLat;
				maxLng = maxLng < markers[ i ].lng ? markers[ i ].lng : maxLng;			
			}
		}
	...
	initCenter: function( mapData ) {
		...
		if ( boundLatLng ) {
			cenLat = this.providerMap.parseValue( boundLatLng.minLat ) + this.providerMap.parseValue( boundLatLng.maxLat );
			cenLng = this.providerMap.parseValue( boundLatLng.minLng ) + this.providerMap.parseValue( boundLatLng.maxLng );
			markersCount = 2;
		}
	...
	triggerMarkerClickEvent: function( marker ) {
		if ( marker ) {
			this.providerMap.triggerMarkerClickEvent( marker );
		}
	},
	...
	addMarkerClickEvent: function( marker, markerData, markerAsEditLink ) { 
		...
		if ( this.providerMap.isClusterHandlerToAdd( markerData ) ) {
			this.providerMap.addOnMarkerClickHandler( marker, function() {
				mapManager.providerMap.clusterHandler( markerData );
			});
			return;
		}
		...
		if ( this.showViewInPopup ) {
			...
			this.providerMap.addOnMarkerClickHandler( marker, function() { 
				var hostPage = Runner.pages.PageManager.getAt( mapManager.tName, mapManager.id ),
					pageParams = Runner.pages.ViewPage.prototype.getOpenPageParams.call( hostPage, markerData.keys, markerData.recId, hostPage.getInlineEditObj(), Runner.emptyFn );
	...
	addMarkerByAddress: function( markerData, mapData ) {		
		...
		this.providerMap.setMarkerByAddress( markerData, mapData, function() {
			if ( !mapManager.isHeatMap( mapData ) ) {
	...
	getMarkerLink: function( keys, markerAsEditLink ) {
		if ( markerAsEditLink && Runner.pages.PageSettings.getTableData(this.tName, "editAvailable") ) {
			return Runner.pages.getUrl( this.tName, Runner.pages.constants.PAGE_EDIT, keys );
		} else if ( Runner.pages.PageSettings.getTableData(this.tName, "viewAvailable") ) {
			return Runner.pages.getUrl( this.tName, Runner.pages.constants.PAGE_VIEW, keys );
		}
	},
	 ...
	 * @param {array} keys
	 */
	refreshMaps: function( recId, keys ) {
		...
		for (mapId in this.mapsData) {
			...
			for (i = 0; i < markers.length; i++) {
				...
				if ( markers[i].lat != '' && markers[i].lng != '' ) {
					this.providerMap.setMapCenter( this.mapsData[ mapId ].map, this.mapsData[ mapId ].zoom, this.providerMap.parseValue( markers[i].lat ), this.providerMap.parseValue( markers[i].lng ) );
					break;
				} 
	 ...
	 */
	initInlineMapOrCenterLink: function( fName, recId, caption, refresh ) {
...
(function() {
	...
	if ( window.settings['global'].mapProvider === Runner.controls.constants.GOOGLE_MAPS ) {
