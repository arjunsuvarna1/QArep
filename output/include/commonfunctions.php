...
function checkTableName($shortTName, $type=false)
{
	...
	if ("testcases" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("testmodule" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("testresult" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("testscenarios" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("test_users" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	...
	if ("admin_users" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("table1" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
{
	$arr = array();
	$strPerm = GetUserPermissions("testcases");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="testcases";
	}
	$strPerm = GetUserPermissions("testmodule");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="testmodule";
	}
	$strPerm = GetUserPermissions("testresult");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="testresult";
	}
	$strPerm = GetUserPermissions("testscenarios");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="testscenarios";
	}
	$strPerm = GetUserPermissions("test_users");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="test_users";
	}
...
function ReadUserPermissions($userID = "")
{
	...
	if($userID != "Guest")
	{
		...
		$sql = "select ".$gConn->addFieldWrappers( "GroupID" )
			...
			." from ". $gConn->addTableWrappers( "qarep_ugmembers" )
	...
	$sql = "select ". $gConn->addFieldWrappers( "TableName" )
		...
		." from ". $gConn->addTableWrappers( "qarep_ugrights" )
...
function AfterFBLogIn( $pUsername, $pPassword, $pDisplayUsername, &$pageObject = null)
{
	...
	$strSQL = "select * from ".$connection->addTableWrappers("test_users")
