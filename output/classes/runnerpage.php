...
class RunnerPage
{
	...
	function __construct(&$params)
	{
		...
		if( $this->pushContext )
			...
			$this->standaloneContext = new RunnerContextItem( CONTEXT_PAGE, array( "pageObj" => $pageObj ));
		...
		if(count($this->allDetailsTablesArr))
		{
			...
			{
				$dPset = new ProjectSettings( $this->allDetailsTablesArr[$i]['dDataSourceTable'] );
	...
	public function displayMasterTableInfo()
	{
		...
		$xt = new Xtempl();
		$xt->eventsObject = getEventObject( $this->masterTable );
		...
		$tc = new TempPageContext( $masterPage );
		...
		$this->addMasterMapsSettings( $masterTableData['mDataSourceTable'], $this->recId + 1, $mrData );
	...
	*/
	...
	function proccessDetailGridInfo(&$record, &$data, $gridRowInd)
	{
		...
		for($i = 0; $i < count($this->allDetailsTablesArr); $i ++)
		{
			...
			$detListAvailabel = $dPset->hasListPage() && $this->permis[$dDataSourceTable]["search"];
			...
			if( ($detailTableData['dispChildCount'] || $detailTableData['hideChild']) && !$this->isDetailTableSubqueryApplied( $dDataSourceTable ) )
			{
		...
		// Issue #12581
		if ( $this->detailsLinksOnList == DL_SINGLE )
		{
	...
	protected function setDetailList( $listTName, $listId, &$data )
	{
		...
		$tc = new TempPageContext( $listPageObject );
	...
	protected function setDetailReportOnEditView( $reportTName, $reportId, &$data  )
	{
		...
		$tc = new TempPageContext( $reportPageObject );
	...
	protected function setDetailChartOnEditView( $chartTName, $chartId, &$data )
	{
		...
		$tc = new TempPageContext( $chartPageObject );
	...
	protected function getPageTitleTemplate( $page, $table, $pSet )
	{
		...
		return RunnerPage::getDefaultPageTitle( $page, $table, $pSet );
	}
	...
	function detailsInGridAvailable()
	{
		for($i = 0; $i < count($this->allDetailsTablesArr); $i ++)
		{
			...
			$detTablePermis = $this->permis[$this->allDetailsTablesArr[$i]['dDataSourceTable']];
			$detListAvailabel = $dPset->hasListPage() && $detTablePermis["search"];
			$detAddAvailabel = $dPset->hasAddPage() && $detTablePermis["add"];
			$detEditAvailabel = $dPset->hasEditPage() && $detTablePermis["edit"];

			if( $detListAvailabel || ( ( $this->detailsLinksOnList == DL_INDIVIDUAL || count($this->allDetailsTablesArr) == 1 )  && ( $detAddAvailabel || $detEditAvailabel ) ) )
			{
				return true;
			}
		}
	...
	public static function sendEmailByTemplate($toEmail, $template, $data)
	{
		...
		return runner_mail( array('to' => $toEmail, 'subject' => $subject, 'body' => $body) );
	}
